...
```mermaid
flowchart TD
  A[确定主题/关键词/人群/目标] --> B[小红书找Top10图文]
  B --> C[拆标题&正文规律 + 点赞高原因]
  C --> D[抓评论并分析情绪共鸣点]
...
## 步骤 0：先明确“写什么”（不问太多，但要抓关键）

从用户输入中提取并记录（缺少就用合理默认）：

- 主题关键词（用于搜Top10与搜图）
- 目标人群（新手/宝妈/学生/打工人…）
- 场景（通勤/宿舍/健身房/周末…）
- 诉求（种草、教程、避坑、对比、清单…）
- 约束（必须包含的词、不能提的点、口吻偏好）
...
- **必须成功获取到 10 条“帖子详情完整可用”的图文帖子**，才能进入步骤 2/3/4/5/6。
- 如果某条帖子“详情获取失败/缺字段/打不开/被风控”，**这条不计入 10 条**，必须继续找下一条补足。
- 允许你累计尝试抓取/解析更多帖子来补齐样本，但**累计尝试上限是 30 条**：
  - 如果累计尝试 **> 30 条** 仍然**拿不到 10 条可用样本**：**立刻停止后续流程**（不允许继续评论分析、背景检索、写作与发布）。
  - 向用户说明：当前关键词/环境/权限导致可用样本不足，并给出最短补救清单（例如：用户提供 10 条对标帖链接/放宽关键词/更换检索词/检查 MCP 登录与权限）。

要求：

- 只选**图文帖**（排除视频/直播切片）
- 相关性优先，其次点赞数高；取**点赞数前10**
...
- 正文规律（段落节奏、清单/步骤、转折、避坑、结尾 CTA）
- 点赞高的原因（至少 3 条，尽量落地到“写法/信息密度/共鸣点/可执行性”）

## 步骤 2：获取第 1 步帖子的评论，并分析“情感共鸣”原因

**目标**：找出观众为什么会“想留言/想转发/想收藏”。

做法：

- 每条 Top 帖子至少抓取前若干条高赞/热评（总量建议 ≥ 30 条）
- 归类评论里的“共鸣触发点”，例如：
  - 共同痛点（我也是…）
  - 身份认同（打工人/学生党/宝妈…）
  - 焦虑与解决（终于有办法/被拯救）
...
当你完成步骤 1+2（且满足“可用样本=10，累计尝试≤30”）后，**必须先按下面结构输出一份总结报告**（内容风格/颗粒度参考用户给的示例），然后在同一轮输出里**自动继续步骤 3→6**。
...
- 用主题关键词在小红书找“点赞 Top10 图文帖”，总结标题/正文规律
- 抓这些帖子的评论，提炼共鸣点与互动问题
